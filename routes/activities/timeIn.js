...
router3.post('/', (request, response, next) => {
    ...
    pool.query(
        ...
        (err, res) => {
            ...
            response.redirect('/timeIn'); //yung route mismo
        }
    );
});

router3.put('/:timeIn', (request, response, next) => {
    const { timeIn } = request.params;
    ...
    fields.forEach((field, index) => {
        pool.query(
            `UPDATE activities SET ${field} = ($1) WHERE timeIn =($2)`, [request.body[field], timeIn],
            (err, res) => {
                if (err) return next(err);

                console.log('UPDATING activity record');

                if (index === fields.length - 1) response.redirect('/timeIn');
            }
        )
    });
});

router3.delete('/:timeIn', (request, response, next) => {
    const { timeIn } = request.params;

    pool.query(
        'DELETE FROM activities WHERE timeIn = ($1)', [timeIn],
        (err, res) => {
            if (err) return next(err);

            response.redirect('/timeIn');
        }
    );
});
