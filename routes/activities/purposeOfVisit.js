...
router3.post('/', (request, response, next) => {
    ...
    pool.query(
        ...
        (err, res) => {
            ...
            response.redirect('/purposeOfVisit'); //yung route mismo
        }
    );
});

router3.put('/:purposeOfVisit', (request, response, next) => {
    const { purposeOfVisit } = request.params;
    ...
    fields.forEach((field, index) => {
        pool.query(
            `UPDATE activities SET ${field} = ($1) WHERE purposeOfVisit =($2)`, [request.body[field], purposeOfVisit],
            (err, res) => {
                if (err) return next(err);

                console.log('UPDATING activity record');

                if (index === fields.length - 1) response.redirect('/purposeOfVisit');
            }
        )
    });
});

router3.delete('/:purposeOfVisit', (request, response, next) => {
    const { purposeOfVisit } = request.params;

    pool.query(
        'DELETE FROM activities WHERE purposeOfVisit = ($1)', [purposeOfVisit],
        (err, res) => {
            if (err) return next(err);

            response.redirect('/purposeOfVisit');
        }
    );
});
